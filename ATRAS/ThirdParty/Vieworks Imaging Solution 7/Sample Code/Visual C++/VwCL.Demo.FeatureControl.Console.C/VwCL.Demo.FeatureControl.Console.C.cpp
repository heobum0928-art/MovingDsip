...
char* Dispatch(char *cmdline, int cmdSize, HCAMERA hCamera)
{
	...
	else if( !(_strnicmp("1", cmdline, 2))) 
	{
		...
		VWSDK::RESULT ret = CameraGetPropertyCount(hCamera, &nPropCount);
		...
		else
		{
			...
			{
				ret = CameraGetPropertyInfoUsingIndex( hCamera, nIdx, &propInfo );
	...
	else if( !(_strnicmp("3", cmdline, 2))) 
	{
		...
		// SetCustomCommand
		VWSDK::RESULT ret = CameraSetCustomCommand( hCamera, caFeature, caArgument );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VWSDK::VwErrorReport(ret));
		}
	...
	else if( !(_strnicmp("4", cmdline, 2))) 
	{
		...
		// GetCustomCommand
		ret = CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
		}
	...
	else if( !(_strnicmp("5", cmdline, 2))) 
	{
		...
		size_t unArgumentSize = sizeof( caArgument );
		// GetCustomCommand
		VWSDK::RESULT ret = CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize, VWSDK::GET_CUSTOM_COMMAND_MAX );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
		}
		...
		ret = CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize, VWSDK::GET_CUSTOM_COMMAND_MIN );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
		}
	...
	else if( !(_strnicmp("6", cmdline, 2))) 
	{
		...
		size_t unArgumentSize = sizeof( caArgument );
		// GetCustomCommand
		ret = CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize, VWSDK::GET_CUSTOM_COMMAND_NUM );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
		}
		...
		for ( UINT i = 0; i < unCount; i ++ )
		{
			unArgumentSize = sizeof( caArgument );
			ZeroMemory( caArgument, unArgumentSize );
			ret = CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize, VWSDK::GET_CUSTOM_COMMAND_INDEX + i );
			if ( ret != VWSDK::RESULT_SUCCESS )
			{
				_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
			}
