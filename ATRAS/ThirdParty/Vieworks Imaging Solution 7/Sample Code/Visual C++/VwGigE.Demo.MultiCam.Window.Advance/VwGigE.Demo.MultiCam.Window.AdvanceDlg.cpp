...
CVwGigEDemoMultiCamWindowAdvanceDlg::CVwGigEDemoMultiCamWindowAdvanceDlg(CWnd* pParent /*=NULL*/)
: CDialog(CVwGigEDemoMultiCamWindowAdvanceDlg::IDD, pParent)
, m_pCamera ( NULL )
...
CVwGigEDemoMultiCamWindowAdvanceDlg::~CVwGigEDemoMultiCamWindowAdvanceDlg()
{
	for( int i = 0; i < DF_DEVICE_COUNT; i ++ )
	{
		if( m_pobjectInfo[i] )
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CVwGigEDemoMultiCamWindowAdvanceDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BUTTON_OPEN1, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedButtonOpen1)
	ON_BN_CLICKED(IDC_BUTTON_PLAY1, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedButtonPlay1)
	ON_BN_CLICKED(IDC_BUTTON_STOP1, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedButtonStop1)
	ON_BN_CLICKED(IDC_BUTTON_CLOSE1, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedButtonClose1)
	ON_EN_CHANGE(IDC_EDIT_IMAGE_BUFFER, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnEnChangeEditImageBuffer)
	ON_BN_CLICKED(IDOK, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedOk)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BTN_SNAP, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedBtnSnap)
	ON_CBN_SELCHANGE(IDC_COMBO_PIXEL_FORMAT, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnCbnSelchangeComboPixelFormat)
	ON_WM_CLOSE()
	ON_CBN_SELCHANGE(IDC_COMBO_PIXEL_FORMAT, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnCbnSelchangeComboPixelFormat)
	ON_CBN_SELCHANGE(IDC_COMBO_PIXEL_SIZE, &CVwGigEDemoMultiCamWindowAdvanceDlg::OnCbnSelchangeComboPixelSize)
	ON_CONTROL_RANGE(BN_CLICKED, IDC_RADIO_DEVICE0, IDC_RADIO_DEVICE3, RadioCtrl)
END_MESSAGE_MAP()


BOOL CVwGigEDemoMultiCamWindowAdvanceDlg::OnInitDialog()
{
...
VWSDK::RESULT CVwGigEDemoMultiCamWindowAdvanceDlg::GetCustomCommand(VWSDK::VwCamera* phCamera, char* cpFeatureName, UINT* unValue, VWSDK::GET_CUSTOM_COMMAND eCmdType)
{
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	CDialog::OnSysCommand(nID, lParam);
}

void CVwGigEDemoMultiCamWindowAdvanceDlg::OnPaint()
{
	if (IsIconic())
	{
		CPaintDC dc(this);
...
HCURSOR CVwGigEDemoMultiCamWindowAdvanceDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedButtonOpen1()
{
	...
	result = m_pvwGigE->OpenCamera( (UINT)m_radioDevice,
		                            &pCamera,
									m_imagebuffernumber,
									0,
									0,
									0,
									m_pobjectInfo[m_radioDevice],
									pImageCallbackFn );
	...
	for (int i = 0; i < nLineupNum; i++){
		...
		VWSDK::RESULT eRet = pCamera->GetCustomCommand("PixelFormat", chResult, &szResult, VWSDK::GET_CUSTOM_COMMAND_INDEX + i);
	...
	// Camera->GetPixelFormatLine
	{
		char chResult[512] = { 0, };
		size_t szResult = sizeof(chResult);
		pCamera->GetEnumPropertyItems("PixelFormat", chResult, &szResult);
	...
	m_pBmpInfo[m_radioDevice]->bmiHeader.biWidth	= nWidth;
	m_pBmpInfo[m_radioDevice]->bmiHeader.biHeight	= (-1 * nHeight);
	//m_pBmpInfo[m_radioDevice]->bmiHeader.biBitCount	= 24;
	m_pBmpInfo[m_radioDevice]->bmiHeader.biClrUsed	= 0;

	m_pBmpInfo[m_radioDevice]->bmiHeader.biBitCount	= CVwResourceType::GetPixelBitCount( pixelFormat );
	...
	for (int i = 0; i<nLineupNum; i++){
		szResult = sizeof(chResult);
		ZeroMemory(chResult, szResult);
		pCamera->GetCustomCommand("PixelSize", chResult, &szResult, VWSDK::GET_CUSTOM_COMMAND_INDEX + i);
		CString strTmp(chResult);
	...
	char chResult[512] = { 0, };
	size_t szResult = sizeof(chResult);
	pCamera->GetEnumPropertyItems("PixelSize", chResult, &szResult);
	...
	UINT tempwidth = 0;
	UINT tempheight = 0;
	GetCustomCommand(pCamera, "Width", &tempwidth);
	GetCustomCommand(pCamera, "Height", &tempheight);
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::GetImageEvent1(VWSDK::OBJECT_INFO* pObjectInfo, VWSDK::IMAGE_INFO* pImageInfo)
{
	CVwGigEDemoMultiCamWindowAdvanceDlg *dlg = (CVwGigEDemoMultiCamWindowAdvanceDlg *)pObjectInfo->pUserPointer;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::GetImageEvent2(VWSDK::OBJECT_INFO* pObjectInfo, VWSDK::IMAGE_INFO* pImageInfo)
{
	CVwGigEDemoMultiCamWindowAdvanceDlg *dlg = (CVwGigEDemoMultiCamWindowAdvanceDlg *)pObjectInfo->pUserPointer;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::GetImageEvent3(VWSDK::OBJECT_INFO* pObjectInfo, VWSDK::IMAGE_INFO* pImageInfo)
{
	CVwGigEDemoMultiCamWindowAdvanceDlg *dlg = (CVwGigEDemoMultiCamWindowAdvanceDlg *)pObjectInfo->pUserPointer;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::GetImageEvent4(VWSDK::OBJECT_INFO* pObjectInfo, VWSDK::IMAGE_INFO* pImageInfo)
{
	CVwGigEDemoMultiCamWindowAdvanceDlg *dlg = (CVwGigEDemoMultiCamWindowAdvanceDlg *)pObjectInfo->pUserPointer;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedButtonPlay1()
{
	if ( NULL == m_pCamera )
	{
		return;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedButtonStop1()
{
	CWaitCursor oWaitCursor;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedButtonClose1()
{
	CWaitCursor oWaitCursor;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnEnChangeEditImageBuffer()
{
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedOk()
{
	if( NULL != m_pCamera )
	{
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnTimer(UINT_PTR nIDEvent)
{
	CString tempstr;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnBnClickedBtnSnap()
{
	if ( NULL == m_pCamera )
	{
		return;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::GetDeviceInfo( int nIndex, CString& strVenderName, CString& strModelName, CString& strDeviceVersion, CString& strDeviceID )
{
	if ( NULL == m_pCamera )
	{
		return;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnCbnSelchangeComboPixelFormat()
{
	CWaitCursor oWaitCursor;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::OnCbnSelchangeComboPixelSize()
{
	if( m_pCamera == NULL )	
		return;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::SetUIResolutionInfo( IN UINT tempwidth, IN UINT tempheight, IN VWSDK::PIXEL_FORMAT ePixelFormat, IN UINT unPixelSize )
{
	int nTmp;
...
int CVwGigEDemoMultiCamWindowAdvanceDlg::GetPixelTypeIndex( IN CString strType )
{
...
CString CVwGigEDemoMultiCamWindowAdvanceDlg::GetPixelTypeStr( IN int nIndex )
{
	CString strTmp;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::MakeUnPackedBuffer()
{
	if ( NULL == m_pCamera )
	{
		// ERROR
	...
	switch( pixelFormat )
	{
		...
		case VWSDK::PIXEL_FORMAT_BAYGR10_PACKED:
			m_pUnpackedImage[m_radioDevice] = new BYTE[nWidth*nHeight*6];
			break;
		case VWSDK::PIXEL_FORMAT_BAYGR8:
		case VWSDK::PIXEL_FORMAT_BAYRG8:
		case VWSDK::PIXEL_FORMAT_MONO16:
			m_pUnpackedImage[m_radioDevice] = new BYTE[nWidth*nHeight*3];
			break;
		default:
			m_pUnpackedImage[m_radioDevice] = new BYTE[nWidth*nHeight*3];
			break;
...
CString CVwGigEDemoMultiCamWindowAdvanceDlg::GetPixelFormatFromEnum( VWSDK::PIXEL_FORMAT pixelFormat )
{
	for ( int i = 0; i < PIXEL_FORMAT_COUNT; i ++ )
	{
...
BOOL CVwGigEDemoMultiCamWindowAdvanceDlg::SetWidthCamera( int nWidth )
{
	UINT nCurrWidth = 0;
...
BOOL CVwGigEDemoMultiCamWindowAdvanceDlg::SetHeightCamera( int nHeight )
{
	UINT nCurrHeight = 0;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::RadioCtrl( UINT ID )
{
	UpdateData( TRUE );
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::SaveDeviceState( int nDeviceNum )
{
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::UpdateDeviceState( int nDeviceNum )
{
	CDeviceState* pDeviceState = &m_deviceState[ nDeviceNum ];
	...
	if ( m_pCamera )
	{
		...
		GetCustomCommand(m_pCamera, "PixelSize", &unPixelSize);
		m_pCamera->GetPixelFormat( &ePixelFormat );
	}
	SetUIResolutionInfo( unWidth, unHeight, ePixelFormat, unPixelSize );
}

void CVwGigEDemoMultiCamWindowAdvanceDlg::OnClose()
{
	if( NULL != m_pCamera )
	{
		AfxMessageBox(_T("First, Close device\n"));
...
int CVwGigEDemoMultiCamWindowAdvanceDlg::GetPixelSizeIndex( int nPixelSize )
{
	// Pixel Size
	int i = 0;
	bool bFnd = false;
	...
	for( i = 0; i < m_cbxPixelSize.GetCount(); i++ ) 
	{
		CString strPixelSize;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::UpdatePixelFormat()
{
	if ( NULL == m_pCamera )
	{
		return;
...
void CVwGigEDemoMultiCamWindowAdvanceDlg::DrawImage( VWSDK::OBJECT_INFO* pObjectInfo, VWSDK::IMAGE_INFO* pImageInfo, int nIndex )
{
	nIndex--;
	...
	CVwGigEDemoMultiCamWindowAdvanceDlg* pDlg = (CVwGigEDemoMultiCamWindowAdvanceDlg*)pObjectInfo->pUserPointer;
	...
	if( liTime.QuadPart - pDlg->m_liLastDisplayTime[nIndex].QuadPart > pDlg->m_nMinInterFrameTime )
	{
		...
		GetCustomCommand(((VWSDK::VwCamera*)pObjectInfo->pVwCamera), "Width", &unWidth);
		GetCustomCommand(((VWSDK::VwCamera*)pObjectInfo->pVwCamera), "Height", &unHeight);
...
BOOL CVwGigEDemoMultiCamWindowAdvanceDlg::PreTranslateMessage(MSG* pMsg)
{
	switch ( pMsg->message )
	{
	case WM_KEYDOWN:
