...
char* Dispatch(char *cmdline, int cmdSize, VWSDK::HCAMERA hCamera);
...
MAIN_MENU:
	...
	while(1)
	{
		...
		{
			Dispatch((char *)chCmdline, sizeof(chCmdline), hCamera);
		}
	}
...
char* Dispatch(char *cmdline, int cmdSize, VWSDK::HCAMERA hCamera)
{
	...
	else if( !(_strnicmp("1", cmdline, 2))) 
	{
		...
		VWSDK::RESULT ret = VWSDK::CameraGetPropertyCount(hCamera, &nPropCount);
		...
		else
		{
			...
			{
				ret = VWSDK::CameraGetPropertyInfoUsingIndex( hCamera, nIdx, &propInfo );
	...
	else if( !(_strnicmp("2", cmdline, 2))) 
	{
		...
		VWSDK::PROPERTY propInfo;
		VWSDK::RESULT ret = CameraGetPropertyInfo( hCamera, caFeature, &propInfo ); 
	...
	else if( !(_strnicmp("3", cmdline, 2))) 
	{
		...
		// SetCustomCommand
		VWSDK::RESULT ret = VWSDK::CameraSetCustomCommand( hCamera, caFeature, caArgument );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VWSDK::VwErrorReport(ret));
		}
	...
	else if( !(_strnicmp("4", cmdline, 2))) 
	{
		...
		// GetCustomCommand
		ret = VWSDK::CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
		}
	...
	else if( !(_strnicmp("5", cmdline, 2))) 
	{
		...
		size_t unArgumentSize = sizeof( caArgument );
		// GetCustomCommand
		VWSDK::RESULT ret = VWSDK::CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize, VWSDK::GET_CUSTOM_COMMAND_MAX );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
		}
		...
		ret = VWSDK::CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize, VWSDK::GET_CUSTOM_COMMAND_MIN );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
		}
	...
	else if( !(_strnicmp("6", cmdline, 2))) 
	{
		...
		size_t unArgumentSize = sizeof( caArgument );
		// GetCustomCommand
		ret = VWSDK::CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize, VWSDK::GET_CUSTOM_COMMAND_NUM );
		if ( ret != VWSDK::RESULT_SUCCESS )
		{
			_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
		}
		...
		for ( UINT i = 0; i < unCount; i ++ )
		{
			unArgumentSize = sizeof( caArgument );
			ZeroMemory( caArgument, unArgumentSize );
			ret = VWSDK::CameraGetCustomCommand( hCamera, caFeature, caArgument, &unArgumentSize, VWSDK::GET_CUSTOM_COMMAND_INDEX + i );
			if ( ret != VWSDK::RESULT_SUCCESS )
			{
				_tprintf(_T("Failed(err:%s))\n"), VwErrorReport(ret));
			}
