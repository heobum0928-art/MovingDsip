#include "stdafx.h"
#include "VwGigE.Demo.SingleCam.Console.h"
// for VwGigE
#include "VwGigE.h"
...
VWSDK::RESULT GetCustomCommand(VWSDK::VwCamera* phCamera, char* cpFeatureName, UINT* unValue, VWSDK::GET_CUSTOM_COMMAND eCmdType = VWSDK::GET_CUSTOM_COMMAND::GET_CUSTOM_COMMAND_VALUE);
...
MAIN_MENU:
	...
	if ( VWSDK::RESULT_SUCCESS != m_pvwGigE.Open() )
	{
		goto MAIN_MENU;
	}
	...
	UINT NICNum = 0;
	VWSDK::RESULT ret = m_pvwGigE.GetNumInterfaces( &NICNum );
	...
	{	
		UINT NICNum = 0;
		UINT deviceOrder = 0;
		m_pvwGigE.GetNumInterfaces( &NICNum );
		for(int i=0;i<(int)NICNum;i++)
		{
			VWSDK::VwInterface* pInterface = NULL;
			m_pvwGigE.OpenInterface( i, &pInterface );

			UINT DeviceNum = 0;
			pInterface->GetNumCameras( &DeviceNum );
			...
			for(int j=0;j<(int)DeviceNum;j++)
			{
				...
				pInterface->GetCameraName( j, chDevName, &sizeDevName );
				printf("** %s\n",chDevName );
...
DEVICE_NUMBER:
	...
	{	
		...
		*/
		...
		if ( VWSDK::RESULT_SUCCESS != m_pvwGigE.OpenCamera( selectednum, &pCamera, 2, 0, 0, 0, pObjectInfo, GetImageEvent1  ) )
		{
		...
		else
		{
			...
			pCamera->GetDeviceVendorName( 0, chVendorName, &sizeVendorName );
			printf("VendorName : %s\n", chVendorName);
			char chModelName[256] = {0,};
			size_t sizeModelName = sizeof( chModelName );
			pCamera->GetDeviceModelName( 0, chModelName, &sizeModelName );
			printf("ModelName : %s\n", chVendorName);
			char chVersion[256] = {0,};
			size_t sizeVersion = sizeof( chVersion );
			pCamera->GetDeviceVersion( 0, chVersion, &sizeVersion );
			printf("Version : %s\n", chVersion);
			UINT nWidth = 0;
			UINT nHeight = 0;
			...
			pCamera->GetCustomCommand("GevMACAddress", chMACAddress, &sizeMAC);
			printf("MACAddress : %s\n", chMACAddress);
		}
	...
	if( m_pvwGigE.Close() != VWSDK::RESULT_SUCCESS )
	{
...
void GetImageEvent1(VWSDK::OBJECT_INFO* pObjectInfo, VWSDK::IMAGE_INFO* pImageInfo)
{
	...
	switch( ePixelFormat )
	{
		...
		case VWSDK::PIXEL_FORMAT_MONO8:
			...
			break;
		case VWSDK::PIXEL_FORMAT_MONO10:
			VWSDK::VwImageProcess::ConvertMono10ToBGR8(PBYTE(vpBuffer), unWidth*unHeight*2, m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_MONO12:
			VWSDK::VwImageProcess::ConvertMono12ToBGR8(PBYTE(vpBuffer), unWidth*unHeight*2, m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_MONO14:
			VWSDK::VwImageProcess::ConvertMono14ToBGR8(PBYTE(vpBuffer), unWidth*unHeight*2, m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_MONO10_PACKED:
		case VWSDK::PIXEL_FORMAT_MONO12_PACKED:
			VWSDK::VwImageProcess::ConvertMonoPackedToBGR8( PBYTE(vpBuffer),
				UINT(1.5*unWidth*unHeight),
				m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_MONO16:
			VWSDK::VwImageProcess::ConvertMono16PackedToBGR8( PBYTE(vpBuffer),
			...
			//-----------------------------------------------------------------
		case VWSDK::PIXEL_FORMAT_BAYGR8:
			VWSDK::VwImageProcess::ConvertBAYGR8ToBGR8( PBYTE(vpBuffer),
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYRG8:
			VWSDK::VwImageProcess::ConvertBAYRG8ToBGR8( PBYTE(vpBuffer),
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYGR10:
			VWSDK::VwImageProcess::ConvertBAYGR10ToBGR8( (WORD*)(vpBuffer),
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYRG10:
			VWSDK::VwImageProcess::ConvertBAYRG10ToBGR8( (WORD*)(vpBuffer),
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYGR12:
			VWSDK::VwImageProcess::ConvertBAYGR12ToBGR8( (WORD*)(vpBuffer),
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYRG12:
			VWSDK::VwImageProcess::ConvertBAYRG12ToBGR8( (WORD*)(vpBuffer),
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYGR10_PACKED:
			VWSDK::VwImageProcess::ConvertMono10PackedToMono16bit( (PBYTE)vpBuffer,
				unWidth,
				unHeight,
				m_bpConvertPixelFormat );
			VWSDK::VwImageProcess::ConvertBAYGR10ToBGR8( (WORD*)m_bpConvertPixelFormat,
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYRG10_PACKED:
			VWSDK::VwImageProcess::ConvertMono10PackedToMono16bit( (PBYTE)vpBuffer,
				...
				unHeight,
				m_bpConvertPixelFormat );
			VWSDK::VwImageProcess::ConvertBAYRG10ToBGR8( (WORD*)m_bpConvertPixelFormat,
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYGR12_PACKED:
			VWSDK::VwImageProcess::ConvertMono12PackedToMono16bit( (PBYTE)vpBuffer,
				unWidth,
				unHeight,
				m_bpConvertPixelFormat );
			VWSDK::VwImageProcess::ConvertBAYGR12ToBGR8( (WORD*)m_bpConvertPixelFormat,
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_BAYRG12_PACKED:
			VWSDK::VwImageProcess::ConvertMono12PackedToMono16bit( (PBYTE)vpBuffer,
				...
				unHeight,
				m_bpConvertPixelFormat );
			VWSDK::VwImageProcess::ConvertBAYRG12ToBGR8( (WORD*)m_bpConvertPixelFormat,
				m_pUnpackedImage,
				unWidth,
				unHeight );
			break;
		case VWSDK::PIXEL_FORMAT_RGB8:
			VWSDK::VwImageProcess::ConvertRGB8ToBGR8( (PBYTE)vpBuffer,
		...
		case VWSDK::PIXEL_FORMAT_BGR8:
			...
			break;
		case VWSDK::PIXEL_FORMAT_RGB12_PACKED:
			VWSDK::VwImageProcess::ConvertRGB12PackedToBGR8( (PBYTE)vpBuffer,
		...
		case VWSDK::PIXEL_FORMAT_BGR12_PACKED:
			...
			break;
		case VWSDK::PIXEL_FORMAT_YUV411:
			VWSDK::VwImageProcess::ConvertYUV411ToBGR8( (PBYTE)vpBuffer,
				UINT(1.5*unWidth*unHeight),
				m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_YUV422_UYVY:
			VWSDK::VwImageProcess::ConvertYUV422_UYVYToBGR8( (PBYTE)vpBuffer,
				unWidth,
				unHeight,
				m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_YUV422_YUYV:
			VWSDK::VwImageProcess::ConvertYUV422_YUYVToBGR8( (PBYTE)vpBuffer,
				unWidth,
				unHeight,
				m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_YUV444:
			VWSDK::VwImageProcess::ConvertYUV444ToBGR8( (PBYTE)vpBuffer,
				UINT(1.5*unWidth*unHeight),
				m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_BGR10V1_PACKED:
		...
		case VWSDK::PIXEL_FORMAT_YUV411_10_PACKED:
		case VWSDK::PIXEL_FORMAT_YUV411_12_PACKED:
			VWSDK::VwImageProcess::ConvertYUV411PackedToBGR8( (PBYTE)vpBuffer,
				UINT(2.25*unWidth*unHeight),
				m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_YUV422_10_PACKED:
		case VWSDK::PIXEL_FORMAT_YUV422_12_PACKED:
			VWSDK::VwImageProcess::ConvertYUV422PackedToBGR8( (PBYTE)vpBuffer,
				UINT(3*unWidth*unHeight),
				m_pUnpackedImage );
			break;
		case VWSDK::PIXEL_FORMAT_PAL_INTERLACED:
...
char* Dispatch(char *cmdline, int cmdSize, VWSDK::VwCamera* phCamera)
{
	...
	else if( !(_strnicmp("3", cmdline, 2))) 
	{
		...
		VWSDK::RESULT ret = phCamera->SetCustomCommand( "Gain", subcmdline );
	...
	else if( !(_strnicmp("4", cmdline, 2))) 
	{
		...
		VWSDK::RESULT ret = phCamera->GetCustomCommand("GainSelector", chResult, &szResult);

		szResult = sizeof(chResult);
		ZeroMemory(chResult, szResult);
		ret = phCamera->GetCustomCommand( "Gain", chResult, &szResult);
		if (VWSDK::RESULT_SUCCESS == ret){
			gain1 = atof(chResult);
			printf("%f\n", gain1);
		}
	...
	else if( !(_strnicmp("5", cmdline, 2))) 
	{
		...
		VWSDK::RESULT ret = phCamera->SetCustomCommand( "ExposureTime", subcmdline );
		if( ret != VWSDK::RESULT_SUCCESS )
		{
			printf("Error code : %d\n",ret);
		}
	...
	else if( !(_strnicmp("6", cmdline, 2))) 
	{
		...
		VWSDK::RESULT ret = phCamera->GetCustomCommand( "ExposureTime", chExposureTime, &szExposureTime );
	...
	else if( !(_strnicmp("7", cmdline, 2))) 
	{
		...
		VWSDK::RESULT ret = phCamera->GetPropertyInfo(subcmdline, &propInfo); 
...
VWSDK::RESULT GetCustomCommand(VWSDK::VwCamera* phCamera, char* cpFeatureName, UINT* unValue, VWSDK::GET_CUSTOM_COMMAND eCmdType)
{
